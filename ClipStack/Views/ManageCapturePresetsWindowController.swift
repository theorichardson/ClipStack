...
@MainActor
final class ManageCapturePresetsWindowController: NSWindowController, NSTableViewDataSource, NSTableViewDelegate {
    private let tableView = NSTableView()
    private let scrollView = NSScrollView()
    private var presets: [CapturePreset] = []
    ...
    convenience init() {
        ...
        window.title = "ClipStack Capture Presets"
        window.center()
    ...
    private func configureContent() {
        ...
        tableView.doubleAction = #selector(showSelectedRegion(_:))
        ...
        let showButton = NSButton(title: "Show Region", target: self, action: #selector(showSelectedRegion(_:)))
        ...
        NSLayoutConstraint.activate([
            ...
            scrollView.bottomAnchor.constraint(equalTo: showButton.topAnchor, constant: -12),

            showButton.leadingAnchor.constraint(equalTo: contentView.leadingAnchor, constant: 16),
            showButton.bottomAnchor.constraint(equalTo: contentView.bottomAnchor, constant: -16),

            deleteButton.leadingAnchor.constraint(equalTo: showButton.trailingAnchor, constant: 8),
            deleteButton.centerYAnchor.constraint(equalTo: showButton.centerYAnchor),

            renameButton.leadingAnchor.constraint(equalTo: deleteButton.trailingAnchor, constant: 8),
            renameButton.centerYAnchor.constraint(equalTo: showButton.centerYAnchor),

            closeButton.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -16),
            closeButton.centerYAnchor.constraint(equalTo: showButton.centerYAnchor),
        ])
    }

    private func reloadPresets() {
        presets = CapturePresetStore.shared.presets
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        cell.textField?.stringValue = "\(preset.name) — \(preset.region.displayDescription)"
        return cell
    }

    @objc private func showSelectedRegion(_ sender: Any) {
    ...
    @objc private func deleteSelected(_ sender: NSButton) {
        let row = tableView.selectedRow
        guard presets.indices.contains(row) else { return }
        CapturePresetStore.shared.delete(id: presets[row].id)
    }
    ...
    private func promptToRename(_ preset: CapturePreset) {
        let alert = NSAlert()
